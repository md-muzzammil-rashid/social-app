...
const PhotoPreview = () => {
    ...
    const getTotalLikes=async()=>{
        const q = query(collection(db,'post',`${postID}/likes`))
    ...
    const likeClickHandler =async()=>{
        ...
        if(currentUser.uid){
            ...
            if(!like){
                addDoc(collection(db,'post',`${postID}/likes`),{
            ...
            if(like){
                ...
                const q = query(collection(db,'post',`${postID}/likes`),where('userId',"==",currentUser.uid))
                const querSnaps = await getDocs(q)
                querSnaps.forEach((l)=>{
                    deleteDoc(doc(db,'post',`${postID}/likes/${l.id}`))
                    setLike(false)
                })
            }
        }
...
const getLike=async()=>{
    const quer = query(collection(db,'post',`${postID}/likes`),where("userId",'==',currentUser.uid))
...
useEffect(()=>{
    ...
    return (
        ...
        <div className='relative w-screen'>
            <img loading='lazy' onDoubleClick={likeClickHandler} src={post.imageLink} className=' w-screen md:w-1/3 object-cover aspect-square' alt="" />
        </div>
        <div className='flex justify-start my-2'>
            {!like?
            ...
            <Link to={`../home/${post.id}/comments`}><SvgIcon sx={{ color: "red" }} className='mx-2' >
